...
const ToDoListProvider = ({ children }: { children: ReactNode }) => {
  ...
  const changeFilter = (type: TaskiFilterType) => setTaskFilter(type);
  ...
  const changeSubtaskContent = (
    ...
    toDoList.find((task) => {
      if (task.id === parentId) {
        return {
          ...task,
          subtasks: task.subtasks.find((subtask) => {
            if (subtask.id === id) {
